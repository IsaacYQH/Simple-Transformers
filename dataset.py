...
class SpeechesClassificationDataset(Dataset):
    ...
    Returns:
        self.class_num (int): number of class in the classification dataset.
    """

    def __init__(self, tokenizer, file_path, class_num):
        ...
        tmp_class_num = len(set([sample[0] for sample in self.samples]))
        assert class_num == tmp_class_num, "number of class mismatch!"
        self.class_num = tmp_class_num
        
        print(f"Dataset loading Done.\nThe number of class is {self.class_num}.")
    ...
    def __getitem__(self, index):
        ...
        # label_tensor = torch.tensor([0]*self.class_num, dtype=torch.long)
